...
public class UserController{
	 ...
	 public String registerForm(Model model) {
		 User user = new User();
		 // 为集合user.courses添加“JAVAEE”和“Spring”，页面的checkbox复选框这两项会被选中
		 List<String> list = new ArrayList<String>();
		 list.add("JAVAEE");
		 list.add("Spring");
		 user.setCourses(list);
		 
		 // 定义页面展现的所有可供选择的复选框内容courseList, 页面上呈现的CheckBox的label与List内值相同
	 ...
	 public String registerForm2(Model model) {
		 ...
		 // 页面展现的可供选择的复选框内容courseMap, key将作为checkbox的value, value作为checkbox的label
	 ...
	 public String registerForm3(Model model) {
		 ...
		 list.add(dept);
		 employee.setDepts(list);  //employee.depts = list
